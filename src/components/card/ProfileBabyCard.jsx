                    ...
                    <>
                        {firstThreeBabies.map((baby, index) => (
                            <div
                                ...
                                className={`flex flex-col @sm:flex-row @sm:items-center @sm:justify-between gap-3 @sm:gap-4 py-4 px-2 -mx-2 rounded-xl transition-colors hover:bg-gray-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-300 ${index !== firstThreeBabies.length - 1 ? "border-b border-gray-200" : ""
                                    }`}
                            >
                                {/* Sección Izquierda: Avatar e Información básica */}
                                <div className="flex items-center gap-4 min-w-0">
                                    <div className="w-14 h-14 @sm:w-16 @sm:h-16 rounded-2xl overflow-hidden bg-gray-100 flex items-center justify-center shrink-0">
                                        {baby.genero === 'M' ? (
                                ...
                                <div className="flex flex-row @sm:flex-col items-center @sm:items-end justify-between @sm:justify-start gap-1 @sm:pl-0 @sm:text-right shrink-0">
                                    <span
                                        className={`px-3 py-0.5 rounded-full text-xs font-semibold whitespace-nowrap ${baby.genero === "M" ? "bg-baby-blue" : "bg-baby-pink"
                                            }`}
                                    >
                                        Nacimiento
                                    </span>
