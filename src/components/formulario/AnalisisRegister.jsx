        ...
        <div className='fixed inset-0 z-50 flex items-center justify-center p-4'>
            ...
            <div className='relative z-10 w-full max-w-md p-6 bg-card rounded-xl shadow-lg border-border flex flex-col gap-4'>
                ...
                <div>
                    ...
                    <p className='text-xs text-muted-foreground mt-1'>Ingresa las métricas correspondientes al control actual.</p>
                </div>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-4'>
                    ...
                    <div className='grid grid-cols-5 gap-4 items-end'>
                        ...
                        <div className='flex flex-col gap-1.5 col-span-3'>
                            <label htmlFor='fecha_control' className='text-xs lg:text-sm font-semibold text-muted-foreground px-1'>Fecha del Control</label>
                            <Input
                                id='fecha_control'
                                type="date"
                        ...
                        <div className='flex flex-col gap-1.5 col-span-2 items-center pb-2'>
                            <label className='text-xs lg:text-sm font-semibold text-muted-foreground px-1 select-none'>¿Realizado?</label>
                            <Controller
                                name="estado"
                                control={control}
                                render={({ field }) => (
                                    <label className="relative inline-flex items-center cursor-pointer select-none mt-1">
                                        ...
                                        />
                                        {/* Cuerpo del Switch */}
                                        <div className="w-11 h-6 bg-muted peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full rtl:peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:border-zinc-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary dark:after:bg-gray-300 dark:after:border-gray-600"></div>
                                    </label>
                                )}
                            />
                        </div>
                    ...
                    {errors.fecha_control && <span className='text-xs text-red-500 dark:text-red-400 px-1 -mt-2'>{errors.fecha_control.message}</span>}

                    {/* CAMPOS EN FILA: PESO Y TALLA */}
                    <div className='grid grid-cols-2 gap-4'>
                        {/* CAMPO: PESO */}
                        <div className='flex flex-col gap-1.5'>
                            <label htmlFor='peso' className='text-xs lg:text-sm font-semibold text-muted-foreground px-1'>Peso (kg)</label>
                            <Input
                                id="peso"
                                type="number"
                                step="0.01"
                                ...
                                {...register('peso')}
                            />
                            {errors.peso && <span className='text-xs text-red-500 dark:text-red-400 px-1'>{errors.peso.message}</span>}
                        </div>

                        {/* CAMPO: TALLA */}
                        <div className='flex flex-col gap-1.5'>
                            <label htmlFor='talla' className='text-xs lg:text-sm font-semibold text-muted-foreground px-1'>Talla (cm)</label>
                            <Input
                                id="talla"
                                type="number"
                                step="0.1"
                                ...
                                {...register('talla')}
                            />
                            {errors.talla && <span className='text-xs text-red-500 dark:text-red-400 px-1'>{errors.talla.message}</span>}
                        </div>
                    </div>

                    {/* CAMPO: DESCRIPCIÓN */}
                    <div className='flex flex-col gap-1.5'>
                        <label htmlFor='descrip' className='text-xs lg:text-sm font-semibold text-muted-foreground px-1'>Descripción / Notas</label>
                        <textarea
                            id="descrip"
                            placeholder="Ej. Control de los 3 meses, vacunas al día."
                            rows={3}
                            {...register('descrip')}
                            className='w-full px-3 py-2 text-sm text-foreground bg-muted border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-ring/50 transition-all resize-none'
                        />
                        {errors.descrip && <span className='text-xs text-red-500 dark:text-red-400 px-1'>{errors.descrip.message}</span>}
                    </div>
